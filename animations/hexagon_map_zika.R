# hexagon map animation for ZIKV
...
zika_map  <- fortify(resulthex, region = "NAME_1")
...
hex_map_data_month <- as.tbl(zika_map) %>%
...
p <- ggplot(hex_map_data_month, aes(long, lat)) +
  ...
  labs(title = "Zika incidence per 100,000 population in 
       {hex_map_data_month$month_name[as.integer(frame_time)]} {hex_map_data_month$year[as.integer(frame_time)]}",
       subtitle = "Colombia") +
  scale_fill_viridis_c(trans = "log",
                       breaks = c(1, 50, 600, 5000, 84175.30),
                       labels = c(0, 0.5, 6, 50, 840)) + 
...
zika_hexagon_map <- animate(p, nframes = 100, fps = 10, height = 600, width = 400)  

# save animation as gif
anim_save("zika_hexagon_map.gif", zika_hexagon_map)
